...
with tab_enfermagem:
    ...
    if not df_clinical_data.empty:
        ...
        # --- Exibição dos Indicadores de Dispositivos ---
        ...
        # --- NOVA SEÇÃO: INDICADORES DE INCIDÊNCIA ---
        st.write("#### Indicadores de Incidência (Evento)")
        
        # --- Cálculo dos Indicadores de Incidência ---
        ...
        col5, col6 = st.columns(2) # Começando uma nova linha de colunas
    ...
    if not df_clinical_data.empty:
        ...
        with col3:
            st.metric(
                ...
                help=f"Total de EOTs Paliativas ({num_eot_pal}) dividido pelo total de EOTs ({num_eot_total_pal}) no período. Filto pela data do evento."
            )
