...
# (Nenhuma alteração nas funções get_project_info, get_project_records, get_project_metadata)
...
with tab2:
    ...
    if st.session_state.get('df_records') is not None:
        ...
        # ### ---------------------------------------------------
        # ### INÍCIO DA SEÇÃO DE GRÁFICOS (MODIFICADA)
        # ### ---------------------------------------------------
        st.divider()
        st.write(f"#### Visualizações para o Ano: {target_year}")

        if col_desnutricao in df_records.columns and col_data in df_records.columns:
            
            # 1. Obter a base de datas (já temos como 'date_series')
            # 2. Obter filtro de desnutrição (já temos como 'cond1_desnutricao')

            # --- Gráfico 1: Contagem de Admissões (Bar Chart) ---
            st.write(f"**Contagem Total de Admissões por Mês**")
            
            # Filtra datas pelo ano
            admissions_in_year = date_series[date_series.dt.year == target_year]
            
            # Conta por mês (garantindo 12 meses)
            admissoes_mes = admissions_in_year.dt.month.value_counts() \
                            .reindex(range(1, 13), fill_value=0) \
                            .sort_index()
            
            # Nomes amigáveis
            admissoes_mes.index = month_names
            admissoes_mes.name = "Contagem de Admissões"
            
            st.bar_chart(admissoes_mes)

            # --- Gráfico 2: Taxa de Desnutrição (Line Chart) ---
            st.write(f"**Taxa de Desnutrição (%) por Mês**")

            # Filtra datas por desnutridos E ano
            desnutridos_in_year_series = date_series[cond1_desnutricao & (date_series.dt.year == target_year)]
            
            # Conta desnutridos por mês
            desnutridos_mes = desnutridos_in_year_series.dt.month.value_counts() \
                              .reindex(range(1, 13), fill_value=0) \
                              .sort_index()
            
            # Calcula a taxa (Numerador / Denominador)
            # (usamos 'admissoes_mes' que calculamos para o gráfico 1)
            taxa_mes = (desnutridos_mes / admissoes_mes) * 100
            
            # Proteção contra divisão por zero (resulta em NaN, que preenchemos com 0)
            taxa_mes = taxa_mes.fillna(0) 
            
            # Nomes amigáveis
            taxa_mes.index = month_names
            taxa_mes.name = "Taxa de Desnutrição (%)"

            st.line_chart(taxa_mes)
            
        else:
            st.info(f"Não é possível gerar os gráficos. Verifique se as colunas '{col_desnutricao}' e '{col_data}' existem.")
