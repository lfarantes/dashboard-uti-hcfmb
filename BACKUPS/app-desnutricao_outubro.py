    ...
    if st.button("Buscar Registros"):
        with st.spinner("Buscando registros... Isso pode levar um momento."):
            ...
            if df_records is not None:
                ...
                # ### ---------------------------------------------------
                # ### INÍCIO DA NOVA SEÇÃO (SEU PEDIDO)
                # ### ---------------------------------------------------
                st.write("#### Análise Filtrada: Admissões em Outubro")
                
                # Definição das nossas variáveis-alvo
                col_desnutricao = 'diagnostico_desnutricao'
                col_data = 'data_e_hora_admissao_uti'
                target_month = 10 # Outubro
                
                # Checagem de robustez: ambas as colunas existem?
                if col_desnutricao in df_records.columns and col_data in df_records.columns:
                    
                    # Condição 1: Desnutrição = 'Sim' (lógica que já temos)
                    cond1_desnutricao = (
                        df_records[col_desnutricao].fillna('').astype(str).str.lower().str.strip() == 'sim'
                    )
                    
                    # Condição 2: Mês = 10 (Outubro)
                    ...
                    # 2. Extrair o mês e comparar
                    #    .dt.month ignora os NaT, que é o que queremos
                    cond2_mes = (date_series.dt.month == target_month)
                    
                    # 3. Combinar as condições (AND = &)
                    combined_filter = (cond1_desnutricao & cond2_mes)
                    
                    # 4. Contar os resultados
                    final_count = combined_filter.sum()
                    
                    # Exibir em colunas para ficar limpo
                    filter_col1, filter_col2, filter_col3 = st.columns(3)
                    with filter_col1:
                        st.metric(label=f"Total de Desnutrição ('Sim')", value=cond1_desnutricao.sum())
                    with filter_col2:
                        st.metric(label=f"Total de Admissões em Outubro", value=cond2_mes.sum())
                    with filter_col3:
                        # Este é o resultado principal do seu pedido
                        st.metric(label="COMBINADO (Desnutrição em Out.)", value=final_count)
