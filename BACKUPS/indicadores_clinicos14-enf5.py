...
def calculate_tempo_ate_meta(df_clinico_raw, selected_month, selected_year):
    ...
    try:
        ...
        try:
            # --- CORREÇÃO AQUI ---
            # Corrigido de 'coluna_data_admissao' para 'col_data_admissao'
            df_geral_paciente[col_data_admissao] = pd.to_datetime(
...
# --- FUNÇÃO 14: TAXA INCIDÊNCIA LPP (A NOVA) ---
def calculate_taxa_incidencia_lpp(df_clinico_raw, selected_month, selected_year):
    ...
    - Filtro: Mês Corrente (baseado na data do evento 'lesao_pressao_data').
    - Numerador: SOMA 'teve_lesao_por_pressao' = '1' NO MÊS.
    - Denominador (Paciente-dias): SOMA 'paciente_ocupando_leito' = '1' NO MÊS.
    """
    
    df = df_clinico_raw.copy()
    
    # --- 1. Cálculo do Denominador (Paciente-Dias) ---
    # (Exatamente como nas funções de utilização de dispositivos)
    ...
    if any(col not in df_geral_paciente.columns for col in [coluna_data_lpp, coluna_lpp_sim_nao]):
        ...
        return 0.0, 0, denominador # Retorna 0 taxa, mas o denominador correto
    ...
    # Filtra os eventos de LPP que ocorreram no mês/ano selecionados
    ...
    # O Numerador é a contagem de 'Sim' ('1') nesses eventos
    numerador = (df_lpp_eventos_mes[coluna_lpp_sim_nao] == '1').sum()

    # --- 3. Resultado ---
    if denominador > 0:
        # O doc diz x100, mas o padrão para densidade de incidência é x1000
        # Vamos seguir o doc da médica, que especifica x100
        taxa = (numerador / denominador) * 100
    else:
        taxa = 0.0 
