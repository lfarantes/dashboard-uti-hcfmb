...
# --- FUNÇÃO 23: TAXA DE REINTERNAÇÃO 48H (A NOVA) ---
def calculate_taxa_reinternacao_48h(df_clinico_raw, selected_month, selected_year):
    ...
    - Filtro: Mês Corrente (baseado na 'data_do_desfecho_uti').
    - Numerador: SOMA 'reinternacao_na_uti_48h' = '1'.
    - Denominador: SOMA 'desfecho_uti' = '1' (Alta).
    ...
    # Denominador: Total de 'Altas' ('1') no período
    denominador = (df_mes_corrente[coluna_desfecho] == '1').sum()
    
    # Numerador: Total de Reinternações ('1') NO MESMO PERÍODO
    # (A lógica da médica é filtrar pela data do desfecho 'Alta', não pela data da reinternação)
