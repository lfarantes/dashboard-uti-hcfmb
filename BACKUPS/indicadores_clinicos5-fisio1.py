...
# --- FUNÇÃO 5: TEMPO MÉDIO DE VM (A NOVA) ---
def calculate_tempo_medio_vm(df_clinico_raw, selected_month, selected_year):
    ...
    - Numerador: SOMA de dias em VM ('suporte_vm_ultimas_24h' = '1').
    - Denominador: CONTAGEM ÚNICA de pacientes que tiveram pelo menos um 'Sim' em VM no mês.
    """
    ...
    coluna_paciente = 'record_id' # Necessário para contagem única
    ...
    # Converter coluna de VM para string
    ...
    # Isolar todos os registros DIÁRIOS daquele mês que tiveram VM = 'Sim' ('1')
    df_dias_em_vm = df_diario_mes[df_diario_mes[coluna_vm] == '1']
    
    # O Numerador é o total de dias em VM (Soma de 'Sim')
    numerador = len(df_dias_em_vm)
    
    # O Denominador é a contagem de pacientes ÚNICOS (nunique) que tiveram VM
