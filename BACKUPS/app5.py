...
# --- Título Principal ---
st.title(f"Sistema de Gestão – UTI Clínica")
...
with tab_medica:
    ...
    if not df_clinical_data.empty:
        ...
        colR1.metric("Mortalidade UTI", f"{taxa_mortalidade:.1f} %", help="Baseado na coorte de admissão com desfecho registrado.")
        colR2.metric("Mortalidade Hospitalar", f"{taxa_mort_hosp:.1f} %", help="Baseado na coorte de admissão com desfecho hospitalar registrado.")
        ...
        # --- EXIBIÇÃO: DESEMPENHO (SMR / SRU) - AGORA POR ÚLTIMO ---
        st.markdown("---")
        st.write("#### 📊 Desempenho Clínico (SMR / SRU)")
        c1, c2, c3, c4, c5 = st.columns(5)
        # Alterado de :.2f para :.6f para precisão de 6 casas decimais
        c1.metric("SMR (Padronizado)", f"{smr_val:.6f}", delta=f"{smr_val - 1:.6f}", delta_color="inverse")
        ...
        with st.expander("🛠️ Prova Real dos Cálculos (Auditoria Detalhada)"):
            prova_real_data = [
                ...
                {"Indicador": "SMR (Padronizado)", "Numerador (N)": f"{tx_obs:.2f}% (Mort. Obs)", "Denominador (D)": f"{tx_esp:.2f}% (Mort. Esp)", "Fórmula": "N / D", "Resultado": f"{smr_val:.2f}"},
