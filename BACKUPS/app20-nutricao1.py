...
# --- Aba 5: Nutrição (COM A NOVA FUNÇÃO) ---
with tab_nutricao:
    st.subheader("Indicadores de Nutrição")
    
    if not df_clinical_data.empty:
        
        # --- Indicador 1 (Já existente) ---
        ...
        col1, col2 = st.columns(2) # Criar colunas para organizar
        ...
        st.markdown("---")
        st.markdown("*Indicador de Tempo até a Meta... em breve.*")
    else:
        st.warning("Não foi possível carregar os dados clínicos.")