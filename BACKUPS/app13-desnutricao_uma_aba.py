    ...
    if st.session_state.get('df_records') is not None:
        ...
        if col_data in df_records.columns and col_desnutricao in df_records.columns:
            ...
            if df_year.empty:
                ...
                denominador_por_mes = df_year['month'].value_counts() \
                                    .reindex(range(1, 13), fill_value=0) \
                                    .sort_index()

                cond_desnutricao_year = (df_year[col_desnutricao].fillna('').astype(str).str.lower().str.strip() == 'sim')
                numerador_por_mes = df_year[cond_desnutricao_year]['month'].value_counts() \
                                    .reindex(range(1, 13), fill_value=0) \
                                    .sort_index()
...
with tab3:
    ...
    if st.session_state.get('df_metadata') is not None:
        ...
        # --- INÍCIO DA CORREÇÃO ---
        # Corrigido de 'df_dashboard' para 'df_metadata'
