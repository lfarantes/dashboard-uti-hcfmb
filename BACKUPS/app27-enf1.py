    ...
    if not df_clinical_data.empty:
        ...
        with col1:
            st.metric(
                ...
                value=f"{taxa_cvc:.1f} %",
                help=f"Total de CVC-dias ({num_cvc_dias}) dividido pelo total de Paciente-dias ({num_pac_dias_cvc}) no período. Filtro pelos registros diários do mês."
            )
        
        st.markdown("---")
        st.markdown("*Indicadores de SVD, Diálise, DVA, LPP e Flebite... em breve.*")
    else:
        st.warning("Não foi possível carregar os dados clínicos.")
    ...
    if not df_clinical_data.empty:
        ...
        with col1:
            st.metric(
                ...
                # --- CORREÇÃO AQUI ---
                # Corrigido de 'num_admisos' para 'num_admissoes'
