...
# --- FUNÇÃO 16: DENSIDADE INFECÇÃO CVC (A NOVA) ---
def calculate_densidade_infeccao_cvc(df_clinico_raw, selected_month, selected_year):
    ...
    - Filtro: Mês Corrente.
    - Numerador: SOMA 'ics_numero' (do geral) onde 'infeccoes_durante_uti_data' está no mês.
    - Denominador (CVC-dias): SOMA 'uso_cvc_nas_ultimas_24h' = '1' (do diário) onde 'data_diario' está no mês.
    - Fórmula: (Numerador / Denominador) * 1000
    """
    
    df = df_clinico_raw.copy()
    
    # --- 1. Cálculo do Denominador (CVC-Dias) ---
    # (Exatamente como na função calculate_taxa_utilizacao_cvc)
    ...
    coluna_data_infeccao = 'infeccoes_durante_uti_data' # Data do evento de infecção
    coluna_ics_numero = 'ics_numero' # O número de infecções de CVC
    ...
    # Converter a contagem de infecções para numérico
    ...
    # Filtra os eventos de Infecção que ocorreram no mês/ano selecionados
    ...
    # O Numerador é a SOMA de 'ics_numero' (que pode ser > 1)
    numerador = df_infeccao_eventos_mes[coluna_ics_numero].sum()

    # --- 3. Resultado ---
    if denominador > 0:
        # Fórmula da médica: (Numerador / Denominador) * 1000
