...
with tab_nutri:
    ...
    if st.session_state.get('df_geral') is not None:
        ...
        # ### ---------------------------------------------------
        # ### MUDANÇA EXECUTADA AQUI (SEU PEDIDO)
        # ### ---------------------------------------------------
        col_data_nutri = 'data_admissao_hospitalar' # <-- Alterado de 'data_e_hora_admissao_uti'
        # ### ---------------------------------------------------

        if col_desnutricao in df_records.columns and col_data_nutri in df_records.columns:

            date_series = pd.to_datetime(df_records[col_data_nutri], errors='coerce')
            ...
            # O denominador aqui é "Admissões no período"
            denominador = (cond2_mes & cond3_ano).sum() 
            # O numerador é "Desnutridos E Admitidos no período"
            ...
            st.warning(f"Não foi possível calcular a análise. Verifique se as colunas '{col_desnutricao}' e '{col_data_nutri}' existem no projeto.")

        st.divider()
        st.write(f"#### Visualização: Taxa de Desnutrição por Mês em {target_year}")
        if col_data_nutri in df_records.columns and col_desnutricao in df_records.columns:
            date_series_full = pd.to_datetime(df_records[col_data_nutri], errors='coerce')
            ...
            if df_year.empty:
                ...
                df_year['month'] = pd.to_datetime(df_year[col_data_nutri], errors='coerce').dt.month
            ...
            st.info(f"Não é possível gerar o gráfico. Colunas '{col_data_nutri}' ou '{col_desnutricao}' não encontradas.")
    else:
        st.info("Por favor, clique no botão 'Buscar Dados de Nutrição / Gerais' para carregar os indicadores.")

# ### ---------------------------------------------------
# ### ABA DE FISIOTERAPIA (Estável)
# ### ---------------------------------------------------
with tab_fisio:
    ...
    if st.session_state.get('df_geral') is None:
        ...
        # --- SEÇÃO INTERATIVA (FISIOTERAPIA) ---
        st.write("#### Análise de Ventilação Mecânica (VM)")
        ...
        month_names_fisio = list(month_map_fisio.keys())

        filter_widget_col1, filter_widget_col2 = st.columns(2)
        with filter_widget_col1:
            current_month_index_fisio = datetime.now().month - 1
            month_name_fisio = st.selectbox(
                "Selecione o Mês",
                options=month_names_fisio,
                index=current_month_index_fisio,
                key='fisio_month'
            )
            target_month_fisio = month_map_fisio[month_name_fisio]
        with filter_widget_col2:
            current_year_fisio = datetime.now().year
            target_year_fisio = st.number_input(
                "Digite o Ano",
                min_value=2010,
                max_value=2050,
                value=current_year_fisio,
                key='fisio_year'
            )

        col_paciente_id = 'record_id'
        col_data_diario = 'data_diario'
        col_data_admissao = 'data_admissao_hospitalar'
        col_vm = 'suporte_vm_ultimas_24h'

        colunas_necessarias_fisio = [col_paciente_id, col_data_diario, col_data_admissao, col_vm]
        if all(col in df_records.columns for col in colunas_necessarias_fisio):

            # --- 1. CÁLCULO DO NUMERADOR ---
            date_series_diario = pd.to_datetime(df_records[col_data_diario], errors='coerce')
            filtro_mes = (date_series_diario.dt.month == target_month_fisio)
            filtro_ano = (date_series_diario.dt.year == target_year_fisio)
            df_filtrado_mes = df_records[filtro_mes & filtro_ano]
            
            vm_padronizado = df_filtrado_mes[col_vm].fillna('').astype(str).str.lower().str.strip()
            numerador_vm = (vm_padronizado == 'sim').sum()

            # --- 2. CÁLCULO DO DENOMINADOR ---
            date_series_admissao = pd.to_datetime(df_records[col_data_admissao], errors='coerce')
            filtro_admissao_mes = (date_series_admissao.dt.month == target_month_fisio)
            filtro_admissao_ano = (date_series_admissao.dt.year == target_year_fisio)
            
            pacientes_admitidos_no_periodo = df_records[filtro_admissao_mes & filtro_admissao_ano][col_paciente_id].unique()
            
            if len(pacientes_admitidos_no_periodo) > 0:
                df_coorte_inteira = df_records[df_records[col_paciente_id].isin(pacientes_admitidos_no_periodo)]
                vm_padronizado_coorte = df_coorte_inteira[col_vm].fillna('').astype(str).str.lower().str.strip()
                pacientes_da_coorte_que_usaram_vm = df_coorte_inteira[vm_padronizado_coorte == 'sim'][col_paciente_id].unique()
                denominador_vm = len(pacientes_da_coorte_que_usaram_vm)
            else:
                denominador_vm = 0

            # --- 3. CÁLCULO DO KPI FINAL ---
            if denominador_vm > 0:
                tempo_medio_vm = numerador_vm / denominador_vm
            else:
                tempo_medio_vm = 0.0

            # --- 4. EXIBIÇÃO DOS KPIs ---
            st.write(f"**Resultados para {month_name_fisio} de {target_year_fisio}**")
            kpi_fisio_1, kpi_fisio_2, kpi_fisio_3 = st.columns(3)

            with kpi_fisio_1:
                st.metric(label="Total de Dias de VM (no Mês)", value=f"{numerador_vm} dias")
            with kpi_fisio_2:
                st.metric(label="Pacientes em VM (Admitidos no Mês)", value=f"{denominador_vm} pacientes")
            with kpi_fisio_3:
                st.metric(label="Tempo Médio de VM (dias)", value=f"{tempo_medio_vm:.2f} dias")

        else:
            st.error(f"Não foi possível calcular o KPI. Verifique se as colunas essenciais existem no Projeto Geral.")
            faltando = [col for col in colunas_necessarias_fisio if col not in df_records.columns]
            st.warning(f"Colunas não encontradas: `{', '.join(faltando)}`")
...
with tab_enf:
    ...
    if st.session_state.get('df_enfermagem') is not None:
        ...
        col_enf = 'numero_enfermagem'
        col_tec_enf = 'numero_tecnico_enfermagem'
        
        colunas_necessarias_enf = [col_data_enf, col_ocupados, col_disponiveis, col_enf, col_tec_enf]
        
        if all(col in df_enf.columns for col in colunas_necessarias_enf):
            ...
            if df_filtrado_enf.empty:
                ...
                st.write(f"**KPIs de Ocupação para {month_name_enf} de {target_year_enf}**")
                
                denominador_ocup = df_filtrado_enf[col_disponiveis].sum()
                numerador_ocup = df_filtrado_enf[col_ocupados].sum()

                if denominador_ocup > 0:
                    taxa_ocupacao_enf = (numerador_ocup / denominador_ocup) * 100
                else:
                    taxa_ocupacao_enf = 0.0

                kpi_enf_1, kpi_enf_2, kpi_enf_3 = st.columns(3)
                with kpi_enf_1:
                    st.metric(label="Leitos-dia Disponíveis (Soma)", value=f"{denominador_ocup:,.0f}")
                with kpi_enf_2:
                    st.metric(label="Leitos-dia Ocupados (Soma)", value=f"{numerador_ocup:,.0f}")
                with kpi_enf_3:
                    st.metric(label="Taxa de Ocupação (%)", value=f"{taxa_ocupacao_enf:.2f} %")

                st.divider() 
                st.write(f"**KPIs de Dimensionamento para {month_name_enf} de {target_year_enf}**")

                profissionais_enf_dia = df_filtrado_enf[col_enf].sum()
                profissionais_tec_dia = df_filtrado_enf[col_tec_enf].sum()
                
                denominador_staff = profissionais_enf_dia + profissionais_tec_dia

                if denominador_staff > 0:
                    ratio_pac_prof = numerador_ocup / denominador_staff
                else:
                    ratio_pac_prof = 0.0

                kpi_staff_1, kpi_staff_2, kpi_staff_3 = st.columns(3)
                with kpi_staff_1:
                    st.metric(label="Pacientes-dia (Soma)", value=f"{numerador_ocup:,.0f}")
                with kpi_staff_2:
                    st.metric(label="Profissionais-dia (Soma Enf+Tec)", value=f"{denominador_staff:,.0f}")
                with kpi_staff_3:
                    st.metric(label="Relação Paciente/Profissional", value=f"{ratio_pac_prof:.2f}")
        else:
            st.error(f"Não foi possível calcular os KPIs. Verifique se as colunas essenciais existem no projeto de Enfermagem.")
            faltando = [col for col in colunas_necessarias_enf if col not in df_enf.columns]
            st.warning(f"Colunas não encontradas: `{', '.join(faltando)}`")
    else:
