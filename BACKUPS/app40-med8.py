...
with tab_medica:
    ...
    if not df_clinical_data.empty:
        ...
        # --- NOVO CÁLCULO ---
        taxa_reint_48h, num_reint, num_altas = indicadores_clinicos26.calculate_taxa_reinternacao_48h(
            df_clinical_data, selected_month, selected_year
        )
        ...
        with col7:
            st.metric(
                ...
                help=f"Total de Reinternações ({num_reint}) dividido pelo total de Altas ({num_altas}) no período. Filtro pela data da alta."
            )
        ...
        st.markdown("---")
        st.markdown("*Indicador de Re-solicitação... em breve.*")
    else:
