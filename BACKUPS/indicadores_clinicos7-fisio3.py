...
# --- FUNÇÃO 7: PROPORÇÃO EOT PALIATIVA (A NOVA) ---
def calculate_taxa_eot_paliativa(df_clinico_raw, selected_month, selected_year):
    ...
    - Filtro: Mês Corrente (baseado na data do evento 'eot').
    - Numerador: SOMA 'eot_paliativa' = '1'.
    - Denominador: SOMA 'eot_sim_nao' = '1'.
    """
    ...
    coluna_data_eot = 'eot' # A data do evento
    coluna_eot_sim_nao = 'eot_sim_nao' # O denominador
    coluna_eot_paliativa = 'eot_paliativa' # O numerador
    ...
    # Converter a coluna de DATA DO EVENTO (eot)
    try:
    ...
    # --- 4. Filtragem (Lógica de "Mês Corrente" no evento EOT) ---
    ...
    # Denominador: Total de EOTs ('eot_sim_nao' = '1') que ocorreram no mês
    denominador = (df_eventos_mes[coluna_eot_sim_nao] == '1').sum()
    
    # Numerador: Total de EOTs Paliativas ('eot_paliativa' = '1') que ocorreram no mês
