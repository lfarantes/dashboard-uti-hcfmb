...
with tab2:
    ...
    # Adicionamos um nome de coluna configurável no topo da aba
    column_to_sum = st.text_input("Nome da Coluna para Somar", "volume_infundido_ml")
    ...
    if st.button("Buscar Registros"):
        with st.spinner("Buscando registros... Isso pode levar um momento."):
            ...
            if df_records is not None:
                ...
                # --- INÍCIO DA NOVA LÓGICA DE SOMA ---
                
                if column_to_sum: # Verifica se o usuário digitou algo
                    if column_to_sum in df_records.columns:
                        # 1. Converte a coluna para numérico de forma segura.
                        # 'errors='coerce'' transforma qualquer texto (ex: 'N/A') em NaN (Not a Number)
                        numeric_column = pd.to_numeric(df_records[column_to_sum], errors='coerce')
                        
                        # 2. Calcula a soma. O .sum() do pandas ignora NaNs por padrão.
                        total_sum = numeric_column.sum()
                        
                        # 3. Exibe a métrica formatada
                        # : ,.2f -> adiciona separador de milhar (,) e formata com 2 casas decimais (.2f)
                        st.metric(
                            label=f"Soma Total de '{column_to_sum}'",
                            value=f"{total_sum:,.2f}" 
                        )
                    else:
                        # Trata o erro de coluna não existente (KeyError)
                        st.warning(f"Atenção: A coluna '{column_to_sum}' não foi encontrada nos dados.")
                
                st.divider() # Adiciona uma linha horizontal
                # --- FIM DA NOVA LÓGICA DE SOMA ---
