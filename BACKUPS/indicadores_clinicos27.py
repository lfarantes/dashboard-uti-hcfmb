...
def calculate_smr(df_raw, selected_month, selected_year):
    ...
    # --- DENOMINADOR: Admitidos no Mês ---
    df_admitidos = df_geral[
        (df_geral[col_admissao].dt.month == selected_month) & 
        (df_geral[col_admissao].dt.year == selected_year)
    ].copy()
    
    # Garantir IDs únicos (Remover duplicatas de record_id)
    ...
    # --- NUMERADOR: Quem desses admitidos já teve alta/óbito/transf hospitalar ---
    ...
    # Cálculo da Mortalidade para o SMR (Óbitos Reais)
    obitos = len(df_saidas[df_saidas[col_desfecho_hosp].astype(str).str.contains('2', na=False)])
    taxa_obs = (obitos / denominador * 100)

    # --- EXPECTATIVA (SAPS) ---
    m_saps = df_admitidos[col_saps].mean()
    ...
    # Retorna 8 valores agora
    return smr, taxa_obs, taxa_esp, m_saps, denominador, numerador_saidas, list_ids_denominador, list_ids_numerador
...
def calculate_sru(df_clinico_raw, selected_month, selected_year):
    ...
    if 'redcap_repeat_instrument' in df.columns:
        ...
        # Garante que não conta o diário
        df_geral = df[df['redcap_repeat_instrument'].isna() | (df['redcap_repeat_instrument'] == '')]

        # Adicione esta linha para garantir que não existam IDs duplicados por erro de exportação
        df_geral = df_geral.drop_duplicates(subset=['record_id'], keep='last')
    else:
        df_geral_paciente = df
