    ...
    if not df_clinical_data.empty:
        ...
        with col5:
            st.metric(
                ...
                help=f"Total de Paciente-dias ({num_pac_dias}) dividido pelo total de Desfechos ({num_desfechos_perm}) no período."
            )
        ...
        st.markdown("---")
        st.markdown("*Indicadores de Mortalidade Hospitalar e Reinternação... em breve.*")
    else:
...
with tab_enfermagem:
    ...
    if not df_clinical_data.empty:
        ...
        # --- Cálculo dos Indicadores de Dispositivos ---
