...
def aplicar_filtro_coorte_desfecho(df, selected_month, selected_year, coluna_desfecho_obrigatorio):
    ...
    que a coluna_desfecho_obrigatorio não esteja vazia.
    """
    df_limpo = df.copy()
...
def calculate_diarias_evitaveis(df_clinico_raw, selected_month, selected_year):
    ...
    # Proteção contra KeyError para a coluna 'data_solicitacao_alta'
...
def calculate_saps3_media(df_clinico_raw, selected_month, selected_year):
    ...
    if coluna_saps_perc in df_admitidos_no_mes.columns:
        df_admitidos_no_mes[coluna_saps_perc] = pd.to_numeric(df_admitidos_no_mes[coluna_saps_perc], errors='coerce')
        media_percentual = df_admitidos_no_mes[coluna_saps_perc].mean()
    else:
        if not pd.isna(media_pontuacao) and media_pontuacao > 0:
            logit = -4.5254 + (0.06877 * media_pontuacao)
            media_percentual = (np.exp(logit) / (1 + np.exp(logit))) * 100
        else:
            media_percentual = 0.0

    return (media_pontuacao if not pd.isna(media_pontuacao) else 0.0, 
            media_percentual if not pd.isna(media_percentual) else 0.0, 
            count_pacientes)
