...
with tab_fisioterapia:
    ...
    if not df_clinical_data.empty:
        ...
        # --- NOVO CÁLCULO ---
        taxa_eot_pal, num_eot_pal, num_eot_total = indicadores_clinicos26.calculate_taxa_eot_paliativa(
            df_clinical_data, selected_month, selected_year
        )

        # --- Exibição dos Indicadores ---
        col1, col2, col3 = st.columns(3) # Criar 3 colunas
        ...
        with col3:
            st.metric(
                ...
                help=f"Total de EOTs Paliativas ({num_eot_pal}) dividido pelo total de EOTs ({num_eot_total}) no período. Filtro pela data do evento."
            )
        
        st.markdown("---")
        st.markdown("*Indicadores de EOT Acidental, Re-IOT... em breve.*")
    else:
        st.warning("Não foi possível carregar os dados clínicos.")
