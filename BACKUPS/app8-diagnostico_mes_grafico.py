...
with tab2:
    ...
    if st.session_state.get('df_records') is not None:
        ...
        # Mapeamento de Mês (movido para cá para ser reutilizado pelo gráfico)
        month_map = {
            'Janeiro': 1, 'Fevereiro': 2, 'Março': 3, 'Abril': 4,
            'Maio': 5, 'Junho': 6, 'Julho': 7, 'Agosto': 8,
            'Setembro': 9, 'Outubro': 10, 'Novembro': 11, 'Dezembro': 12
        }
        month_names = list(month_map.keys()) # Lista ['Janeiro', 'Fevereiro', ...]
        ...
        if col_desnutricao in df_records.columns and col_data in df_records.columns:
            ...
            st.warning(f"Não foi possível calcular a análise filtrada. Verifique as colunas '{col_desnutricao}' e '{col_data}' existem no projeto.")
        ...
        if col_data in df_records.columns:
            # 1. Garante que a coluna de data esteja no formato correto
            date_series = pd.to_datetime(df_records[col_data], errors='coerce')
            
            # 2. Filtra as datas APENAS pelo ano selecionado
            admissions_in_year = date_series[date_series.dt.year == target_year]
            
            # 3. Conta as ocorrências de cada mês
            # .reindex(range(1, 13), ...) é o truque para garantir os 12 meses
            month_counts = admissions_in_year.dt.month.value_counts() \
                            .reindex(range(1, 13), fill_value=0) \
                            .sort_index()
            
            # 4. Define os nomes amigáveis no eixo
