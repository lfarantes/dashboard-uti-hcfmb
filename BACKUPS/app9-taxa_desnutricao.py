...
with tab2:
    ...
    if st.session_state.get('df_records') is not None:
        ...
        if col_desnutricao in df_records.columns and col_data in df_records.columns:
            ...
            # Converter a coluna de data (uma única vez)
            ...
            # ### ---------------------------------------------------
            # ### INÍCIO DA LÓGICA DO SEU PEDIDO
            # ### ---------------------------------------------------

            # Cálculo dos números para a Taxa
            # Denominador: Admissões no período (Mês E Ano)
            denominador = (cond2_mes & cond3_ano).sum()
            
            # Numerador: Desnutridos (Cond1) E no Mês (Cond2) E no Ano (Cond3)
            numerador = (cond1_desnutricao & cond2_mes & cond3_ano).sum()
            
            # Cálculo da Taxa (com proteção contra divisão por zero)
            if denominador > 0:
                taxa_desnutricao = (numerador / denominador) * 100
            else:
                taxa_desnutricao = 0.0 # Define como float
        ...
        # --- SEÇÃO DE GRÁFICO (sem alteração) ---
        st.divider()
