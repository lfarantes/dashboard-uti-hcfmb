    ...
    if not df_clinical_data.empty:
        ...
        with col2:
            st.metric(
                ...
                help=f"Total de VM-dias ({num_vm_dias}) dividido pelo total de Paciente-dias ({num_pac_dias}) no período. Filtro pelos registros diários do mês."
            )
        
        st.markdown("---")
        st.markdown("*Indicadores de EOT, Re-IOT... em breve.*")
    else:
        st.warning("Não foi possível carregar os dados clínicos.")
