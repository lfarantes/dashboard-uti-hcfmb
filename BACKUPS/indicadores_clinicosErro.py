...
def _calcular_probabilidade_saps3(pontos):
    """
    Aplica a Equação Padrão conforme a planilha do usuário.
    Logit = -32.6659 + ln(SAPS3 + 20.5958) * 7.3068
    """
    if pd.isna(pontos) or pontos <= 0:
        return np.nan
    try:
        logit = -32.6659 + (np.log(pontos + 20.5958) * 7.3068)
        prob = (np.exp(logit) / (1 + np.exp(logit))) * 100
        return prob
    except:
        return np.nan
...
def calculate_saps3_media(df_clinico_raw, selected_month, selected_year):
    logging.info(f"Iniciando cálculo de SAPS 3 Média para {selected_month}/{selected_year}")
    df = df_clinico_raw.copy()
    
    if 'redcap_repeat_instrument' in df.columns:
        df = df[(df['redcap_repeat_instrument'].isnull()) | (df['redcap_repeat_instrument'] == '')]

    df['data_adm'] = pd.to_datetime(df['data_e_hora_admissao_uti'], errors='coerce')
    df_mes = df[(df['data_adm'].dt.month == selected_month) & (df['data_adm'].dt.year == selected_year)].copy()
    
    if df_mes.empty:
        return 0.0, 0.0, 0

    df_mes['saps_pts'] = pd.to_numeric(df_mes['saps_3_pontuacao'], errors='coerce')
    # Aplica a fórmula exata da planilha
    df_mes['saps_calc_perc'] = df_mes['saps_pts'].apply(_calcular_probabilidade_saps3)
    
    media_pts = df_mes['saps_pts'].mean()
    media_perc = df_mes['saps_calc_perc'].mean()
    count = len(df_mes.dropna(subset=['saps_pts']))
    
    return (media_pts if not pd.isna(media_pts) else 0.0), (media_perc if not pd.isna(media_perc) else 0.0), count
...
def calculate_smr(df_clinico_raw, selected_month, selected_year):
    logging.info(f"Iniciando cálculo de SMR para {selected_month}/{selected_year}")
    df = df_clinico_raw.copy()
    
    if 'redcap_repeat_instrument' in df.columns:
        df = df[(df['redcap_repeat_instrument'].isnull()) | (df['redcap_repeat_instrument'] == '')]

    df['data_hosp'] = pd.to_datetime(df['data_desfecho_hospitalar'], errors='coerce')
    df_mes = df[(df['data_hosp'].dt.month == selected_month) & (df['data_hosp'].dt.year == selected_year)].copy()
    
    if df_mes.empty:
        return 0.0, 0.0, 0.0
    
    # Mortalidade Observada
    num_obs = (df_mes['desfecho_hospitalar'].astype(str).str.contains('2', na=False)).sum()
    tx_obs = (num_obs / len(df_mes) * 100)
    
    # Mortalidade Esperada usando a fórmula da planilha
    df_mes['saps_pts'] = pd.to_numeric(df_mes['saps_3_pontuacao'], errors='coerce')
    df_mes['prob_calc'] = df_mes['saps_pts'].apply(_calcular_probabilidade_saps3)
    tx_esp = df_mes['prob_calc'].mean()
    
    smr = tx_obs / tx_esp if (not pd.isna(tx_esp) and tx_esp > 0) else 0.0
    return smr, tx_obs, (tx_esp if not pd.isna(tx_esp) else 0.0)
