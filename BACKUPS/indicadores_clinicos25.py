...
# --- FUNÇÃO 25: SMR (A NOVA) ---
def calculate_smr(df_clinico_raw, selected_month, selected_year):
    """
    Calcula a Taxa de Mortalidade Padronizada (SMR).
    Lógica (baseada no doc):
    - Filtro: Mês do DESFECHO HOSPITALAR.
    - Taxa Observada: (Óbitos Hospitalares / Total Desfechos Hospitalares) * 100
    - Taxa Esperada: Média(SAPS-3 %) para a mesma coorte.
    - SMR: Taxa Observada / Taxa Esperada
    """
    ...
    logging.info(f"SMR: Filtrado para {len(df_geral_paciente)} registros 'geral_paciente'.")

    # --- 2. Definição das Colunas ---
    col_data_desfecho_hosp = 'data_desfecho_hospitalar'
    col_desfecho_hosp = 'desfecho_hospitalar'
    col_saps_perc = 'saps_3' # 'SAPS-3 (%)'

    cols_necessarias = [col_data_desfecho_hosp, col_desfecho_hosp, col_saps_perc]
    if any(col not in df_geral_paciente.columns for col in cols_necessarias):
        logging.warning(f"Colunas do SMR ({cols_necessarias}) não encontradas. Retornando 0.")
        return 0.0, 0.0, 0.0

    # --- 3. Preparação dos Dados ---
    df_preparado = df_geral_paciente.copy()
    try:
        df_preparado[col_data_desfecho_hosp] = pd.to_datetime(
            df_preparado[col_data_desfecho_hosp].str.strip(), 
            format='%Y-%m-%d', 
            errors='coerce'
        )
    except Exception:
        try:
            ...
            logging.error(f"Erro ao converter data do desfecho hospitalar (SMR): {e}. Verifique o formato.")
            return 0.0, 0.0, 0.0

    df_preparado[col_desfecho_hosp] = df_preparado[col_desfecho_hosp].astype(str).str.replace(r'\.0$', '', regex=True)
    df_preparado[col_saps_perc] = pd.to_numeric(df_preparado[col_saps_perc], errors='coerce')

    # --- 4. Filtragem (Coorte do Mês de Desfecho Hospitalar) ---
    df_com_desfecho = df_preparado.dropna(subset=[col_data_desfecho_hosp])
    
    df_mes_corrente = df_com_desfecho[
        (df_com_desfecho[col_data_desfecho_hosp].dt.month == selected_month) &
        (df_com_desfecho[col_data_desfecho_hosp].dt.year == selected_year)
    ]
    
    if df_mes_corrente.empty:
        logging.info("SMR: Nenhum desfecho hospitalar no mês.")
        return 0.0, 0.0, 0.0

    # --- 5. Cálculo da Taxa Observada (Numerador do SMR) ---
    denominador_observado = len(df_mes_corrente)
    
    is_obito_raw_1 = df_mes_corrente[col_desfecho_hosp].eq('2')
    is_obito_raw_2 = df_mes_corrente[col_desfecho_hosp].eq('2.0')
    numerador_observado = (is_obito_raw_1 | is_obito_raw_2).sum()

    if denominador_observado > 0:
        taxa_observada = (numerador_observado / denominador_observado) * 100
    else:
        taxa_observada = 0.0

    # --- 6. Cálculo da Taxa Esperada (Denominador do SMR) ---
    taxa_esperada = df_mes_corrente[col_saps_perc].mean()
    if pd.isna(taxa_esperada):
        taxa_esperada = 0.0

    # --- 7. Cálculo do SMR ---
    if taxa_esperada > 0:
        smr = taxa_observada / taxa_esperada
    else:
        smr = 0.0 # Evita divisão por zero

    logging.info(f"Cálculo SMR (Evento): SMR={smr:.2f}, Observada={taxa_observada:.1f}%, Esperada={taxa_esperada:.1f}%")
    
    return smr, taxa_observada, taxa_esperada